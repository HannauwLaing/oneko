...
public class NekoController {
	...
	public NekoController(NekoSettings settings, JWindow invisibleWindow, JFrame visibleWindow, JLabel free,
			JLabel boxed) {
	...
	public void locateMouseAndAnimateCat() {
		...
		if (settings.getDoRandMov() == 0) {
			...
			if (this.randomMoveCounter-- <= 0) {
				...
				while (Math.abs(newX - this.prevX) < 450 && Math.abs(newY - this.prevY) < 275) {
					newX = rand.nextInt(nekoBounds.width) + nekoBounds.x;
					newY = rand.nextInt(nekoBounds.height) + nekoBounds.y;

				}

				this.prevX = newX;
				this.prevY = newY;
				// System.out.println(this.prevX);
				// System.out.println(this.prevY);
			}
	...
	public void resetRandMovCounter() {
		this.randomMoveCounter = 0;
	}
